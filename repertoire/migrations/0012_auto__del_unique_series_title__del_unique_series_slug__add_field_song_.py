...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Removing unique constraint on 'Series', fields ['slug']
        db.delete_unique('repertoire_series', ['slug'])

        # Removing unique constraint on 'Series', fields ['title']
        db.delete_unique('repertoire_series', ['title'])

        # Adding field 'Song.audio_file'
        db.add_column('repertoire_song', 'audio_file', self.gf('django.db.models.fields.files.FileField')(max_length=100, null=True, blank=True), keep_default=False)

        # Adding field 'PerformedSong.note'
        db.add_column('repertoire_performedsong', 'note', self.gf('django.db.models.fields.TextField')(null=True, blank=True), keep_default=False)


    def backwards(self, orm):
        
        # Adding unique constraint on 'Series', fields ['title']
        db.create_unique('repertoire_series', ['title'])

        # Adding unique constraint on 'Series', fields ['slug']
        db.create_unique('repertoire_series', ['slug'])

        # Deleting field 'Song.audio_file'
        db.delete_column('repertoire_song', 'audio_file')

        # Deleting field 'PerformedSong.note'
        db.delete_column('repertoire_performedsong', 'note')
